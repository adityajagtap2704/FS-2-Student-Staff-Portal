...
export default function NotesClient({
  ...
  const handleDownload = async (noteId: number, fileName: string) => {
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        toast.error("Download failed", data.error || "Could not download file.");
      }
    } catch (err) {
      console.error(err);
      toast.error("Download Error", "Connection refused.");
    }
  };
  ...
  const getFileStyle = (fileName: string) => {
    ...
    switch (ext) {
      case "pdf":
        return { icon: FileText, bg: "bg-red-50 border-red-100", color: "text-red-500", badge: "danger" as const };
      case "ppt":
      case "pptx":
        return { icon: Monitor, bg: "bg-orange-50 border-orange-100", color: "text-orange-500", badge: "warning" as const };
      case "doc":
      case "docx":
        return { icon: FileText, bg: "bg-blue-50 border-blue-100", color: "text-blue-500", badge: "info" as const };
      case "png":
      case "jpg":
      case "jpeg":
      case "webp":
        return { icon: Eye, bg: "bg-purple-50 border-purple-100", color: "text-purple-500", badge: "neutral" as const };
      default:
        return { icon: FileDown, bg: "bg-gray-50 border-gray-100", color: "text-gray-500", badge: "neutral" as const };
    }
  };

  // ── Form Subject Options Helper ──────────────────────────────────────
  const getFormSubjectOptions = (): string[] => {
    const opts = subjects.map((s) => s.name);
    opts.push("Other");
    return opts;
  };

  // ── Form Semester Options Helper ─────────────────────────────────────
  const getFormSemesterOptions = (): string[] => {
    const opts = ["All", ...semesters];
    opts.push("Other");
    return opts;
  };
